...
export function calculateProgress(bonus: Bonus): number {
  ...
  switch (depositReq.type) {
    ...
    case 'both':
      ...
      const eachProgress = deposits.every(d => d.amount >= (depositReq.eachAmount || 0)) ? 1 : 0;
...
export function getRemainingDays(bonus: Bonus): number {
  ...
  const endDate = new Date(startDate.getTime() + bonus.requirements.timeFrame * 24 * 60 * 60 * 1000);
...
export function canWithdraw(bonus: Bonus): boolean {
  if (!isCompleted(bonus)) return false;
  if (!bonus.requirements.holdPeriod) return true;
  
  const startDate = new Date(bonus.startDate);
  const holdEndDate = new Date(startDate.getTime() + bonus.requirements.holdPeriod * 24 * 60 * 60 * 1000);
...
export function calculateRemainingAmount(bonus: Bonus): number {
  ...
  switch (depositReq.type) {
    ...
    case 'each':
      const remainingDeposits = Math.max((depositReq.count || 0) - deposits.length, 0);
      return remainingDeposits * (depositReq.eachAmount || 0);
    case 'both':
      const remainingTotal = Math.max((depositReq.totalAmount || 0) - totalDeposited, 0);
      const remainingEach = Math.max((depositReq.count || 0) - deposits.length, 0) * (depositReq.eachAmount || 0);
